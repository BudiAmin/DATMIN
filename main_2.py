...
class GoogleSearchUI(QWidget):
    ...
    def load_documents(self):
        ...
        doc_tfs = [
            term_frequency(tokens, vocab) for tokens in tokens
        ]  # This will create a dictionary for each doc
        return docs, doc_tfs, vocab
    ...
    def process_query(self):
        ...
        similarities = []
        vsm_details = []  # Menyimpan detail VSM per cuplikan dokumen
        threshold = 0.1  # Nilai ambang batas similarity (misalnya 0.1)

        # Menampilkan term frequency (TF)
        tf_table_text = "Term Frequency (TF) pada dokumen:\n"
        tf_table_text += "+------------+---------+---------+---------+\n"
        tf_table_text += "| Term       |   Doc 1 |   Doc 2 |   Doc 3 |\n"
        tf_table_text += "+============+=========+=========+=========+\n"

        for term in self.vocabulary:
            tf_table_text += f"| {term:<10} |"
            for doc_tf in self.doc_tokens:
                tf_table_text += f"   {doc_tf.get(term, 0):<6} |"
            tf_table_text += "\n"

        tf_table_text += "+------------+---------+---------+---------+\n"
        self.output_vsm.setText(tf_table_text)

        for i, tokens in enumerate(self.doc_tokens):
            doc_tf = term_frequency(tokens, self.vocabulary)

            # Perhitungan Cosine Similarity
            dot_product = sum(query_tf[term] * doc_tf.get(term, 0) for term in query_tf)
            magnitude_query = math.sqrt(sum(query_tf[term] ** 2 for term in query_tf))
            magnitude_doc = math.sqrt(sum(doc_tf[term] ** 2 for term in doc_tf))

            cosine_sim = (
                dot_product / (magnitude_query * magnitude_doc)
                if magnitude_query and magnitude_doc
                else 0
            )
            ...
            vsm_details.append(
                {
                    "doc": os.path.basename(self.files[i]),
                    "dot_product": dot_product,
                    "magnitude_query": magnitude_query,
                    "magnitude_doc": magnitude_doc,
                    "cosine_sim": cosine_sim,
                }
            )

            if cosine_sim > threshold:  # Hanya dokumen dengan similarity > threshold
                snippets = highlight_query(self.documents[i], query_tokens)
                similarities.append(
                    (self.files[i], cosine_sim, self.documents[i], snippets)
                )

        # Menampilkan hasil perhitungan di QTextEdit
        vsm_text = ""
        for detail in vsm_details:
            vsm_text += f"\nPerhitungan untuk {detail['doc']}:\n"
            vsm_text += "=== Perhitungan Cosine Similarity ===\n"
            vsm_text += f"Dot Product = {detail['dot_product']}\n"
            vsm_text += (
                f"Magnitude Vector Query = √"
                + " + ".join([f"{query_tf[term]}^2" for term in query_tf])
                + f" = {round(detail['magnitude_query'], 4)}\n"
            )
            vsm_text += (
                f"Magnitude Vector Dokumen = √"
                + " + ".join([f"{doc_tf.get(term, 0)}^2" for term in doc_tf])
                + f" = {round(detail['magnitude_doc'], 4)}\n"
            )
            vsm_text += f"Cosine Similarity = {round(detail['cosine_sim'], 4)}\n"
            vsm_text += "=====================================\n"

        self.output_vsm.setText(vsm_text)

        similarities.sort(key=lambda x: x[1], reverse=True)

        self.table_result.setRowCount(len(similarities))
        for idx, (doc, sim, _, _) in enumerate(similarities):
        ...
        if similarities:
            ...
            similarity, content, snippet = best_match[1], best_match[2], best_match[3]
